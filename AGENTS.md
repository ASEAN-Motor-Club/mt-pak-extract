...
- **Dump-space cargo visuals = DumpCargoSurfaceMesh + DumpCargoSurfaceMaterial**: dump cargos do NOT render the actor mesh in the bed — the game projects these two import refs (StaticMesh + Material/MaterialInstanceConstant) over the dump volume. Null refs = invisible cargo (hit 2026-09-21, CocaPaste in schedule-i ≤ v0.4.19). `create_cargopack.py` takes per-entry `dump_mesh_path`/`dump_material_path` (must point at assets already in the game pak, e.g. Coal's). For vanilla dump particles/sound also clone the actor from `out/VolumeTypeCargo_<X>.uasset` (`blueprint_template` + matching `clone_old_name`). Round-trip verify: `mod_explore --extract` both child-table parts next to each other, then `UAssetTool --dump <ABSOLUTE path>` (it resolves against repo RootDir, not CWD) and check the row's `DumpCargoSurface*` indices are negative (import refs), not 0.
